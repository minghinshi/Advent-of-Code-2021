...
class DeterministicDice {
    ...
    public void stepAll() {
        while (player1Score < 1000 && player2Score < 1000) step();
    }
...
class DiracDice {
    ...
    public void stepAll() {
        while (numberOfStates.size() != 0) step();
    }
        ...
        for (int key : numberOfStates.keySet()) {
            for (int moves : movesToCases.keySet()) {
                ...
                if (newState[scorePointer] >= winningScore) {
                    if (isPlayer1Turn) player1Wins += numberOfStates.get(key) * movesToCases.get(moves);
                    else player2Wins += numberOfStates.get(key) * movesToCases.get(moves);
                } else {
